export class Popup {
  ...
  protected initUserControl(user: HTMLDivElement): void {
    user.appendChild(document.createTextNode("override me"));
  }

  private initDialogElement(dialog: HTMLElement): void {
    dialog.appendChild(this.addCloseElement())
    ...
    dialog.appendChild(this.addButton("Cancel"))
      .addEventListener("click", this.hide.bind(this));
  ...
  private addButton(textContent: string, ...token: string[]): HTMLButtonElement {
